...
# Knowledge Corpus Mode pilot: seminar/training materials.
...
_FIXTURE_TEMPLATE = """\
# TIGA Hunt - Knowledge Corpus eval fixture
# Replace expected_paths / expected_citations with real files after first indexing.
# Run: python tiga.py eval
#
# expected_paths: suffix-match against indexed file_path (partial paths OK).
# expected_citations: optional rel/path#chunk refs for stricter citation checks.
# answer_must_include: optional terms/claims for groundedness review.

- query: "What recurring themes were discussed across seminars?"
  expected_paths:
    - ""
  answer_must_include: []

- query: "Which talks mentioned AI in architecture workflows?"
  expected_paths:
    - ""
  answer_must_include: [AI]

- query: "Summarize what this corpus says about design process and collaboration."
  expected_paths:
    - ""
  answer_must_include: [design, collaboration]

- query: "Which speaker discussed fire safety, regulations, or compliance?"
  expected_paths:
    - ""
  answer_must_include: [fire, regulation, compliance]

- query: "What are the most repeated ideas across all training material?"
  expected_paths:
    - ""
  answer_must_include: []
"""
...
def cmd_wiki(_args: argparse.Namespace) -> None:
    """Build durable Knowledge Corpus Mode wiki pages."""
    from config import cfg
    from core.db import get_connection
    from core.wiki import build_wiki

    cfg.ensure_dirs()
    conn = get_connection(cfg.get_db_path())
    try:
        stats = build_wiki(cfg_obj=cfg, conn=conn)
    finally:
        conn.close()
    print(json.dumps(stats, indent=2))
    print(f"wiki_dir: {cfg.work_dir / 'wiki'}")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    # wiki
    sub.add_parser("wiki", help="Build Knowledge Corpus Mode markdown wiki")
